...
-- | The Idris compiler style
module HIndent.Styles.Idris where
...
idris :: Style
idris =
  Style {styleName = "idris"
        ,styleAuthor = "The Idris Community"
        ,styleDescription = "The style for Idris."
        ...
        depend (write (case boxed of
               ...
               (do (fits,_) <- fitsOnOneLine p
                   ...
                   write (case boxed of
                            Unboxed -> "#)"
                            Boxed -> ")"))
        where p = inter (comma >> space) (map pretty tys)
...
decl (TypeDecl _ head ty) =
  do write "type "
     pretty head
     write " ="
     (fits,st) <- fitsOnOneLine (space >> pretty ty)
     if fits
        then put st
        else do newline
             ...
             if fits
                then put st
                       ...
                       tys ->
                         prefixedLined "-> "
                                       (map pretty tys)
decl (DataDecl _ dataOrNew Nothing declHead constructors mayDeriving) =
            ...
            (c:cs) ->
              depend space $
                do write "="
                   ...
                   forM_ cs $ \ctor ->
                     do newline
                        depend (write "|" >> space) (pretty ctor)
     forM_ mayDeriving $ \deriv ->
       do newline
          indented indentSpaces (pretty deriv)
decl e = prettyNoExt e

derivings :: Deriving NodeInfo -> Printer t ()
derivings (Deriving _ heads)=
  do write "deriving"
     space
     if (length heads == 1)
       then pretty (head heads)
       else parens (inter (comma >> space) (map pretty heads))
  ...
  depend (write (case boxed of
         ...
         (do (fits,_) <- fitsOnOneLine p
             ...
             write (case boxed of
                      Unboxed -> "#)"
                      Boxed -> ")"))
  where p = inter (comma >> space) (map pretty exps)
exp (List _ es) =
  do (ok,st) <- sandbox renderFlat
     if ok
        then put st
        else depend (write "[ ")
                    (do suffixedLined "," (map pretty es)
                        newline
                        write "]")
  where renderFlat =
          do line <- gets psLine
        ...
        lined =
          do newline
             ...
             prefixedLined
               ...
               (map pretty qstmt)
exp (If _ c t f) =
  do depend (write "if" >> space) (pretty c)
     newline
     indented 2 $
       do depend (write "then" >> space) (pretty t)
          newline
          depend (write "else" >> space) (pretty f)
exp e = prettyNoExt e

pattern :: Pat NodeInfo -> Printer t ()
pattern (PTuple _ boxed pats) =
  depend (write (case boxed of
                   Unboxed -> "(#"
                   Boxed -> "("))
         (do (fits,_) <- fitsOnOneLine p
             ...
             write (case boxed of
                      Unboxed -> "#)"
                      Boxed -> ")"))
  where p = inter (comma >> space) (map pretty pats)
pattern other = prettyNoExt other


pragma :: ModulePragma NodeInfo -> Printer t ()
pragma (LanguagePragma _ names) =
  do depend (write "{-# LANGUAGE ") $
       suffixedLined "," (map pretty names)
     space
     write "#-}"
pragma other = prettyNoExt other
...
dependOrNewline :: Printer t ()
                ...
                -> Printer t ()
dependOrNewline left right sep f =
  do (fits,st) <- fitsOnOneLine (depend (left >> sep) (f right))
     if fits
       then put st
       else do left
               newline
...
condecls :: ConDecl NodeInfo -> Printer t ()
condecls (ConDecl _ name bangty) =
  depend (pretty name) $
    forM_ bangty $ \ty -> space >> pretty ty
condecls (RecDecl _ name fields) =
  do depend (pretty name >> space) $
       do depend (write "{ ") (printFields fields)
  ...
  where
    printFields [] = return ()
    printFields [f] = fieldDecl True f
    printFields (f:fs) = fieldDecl False f >> printFields fs

    fieldDecl :: Bool -> FieldDecl NodeInfo -> Printer t ()
    fieldDecl done (FieldDecl (NodeInfo _ cs) names ty) =
      do (sequence_ (intersperse (write ", ") (map pretty names)))
         write " :: "
         pretty ty
         unless done $
           do comma
              newline
         forM_ cs $ \(ComInfo c _) ->
           printComment Nothing c
